#Raspador de texto das Emendas Parlamentares do DF 2012 para o DeOlhoNasEmendas - TXT2CSV Ushahidi
# Autor:
...
import re
import scraperwiki
url_base = "http://adoteumdistrital.com.br/arquivos/dcl-2011-11-23-suplemento"

def inicializaUmArquivo(urlComumAtodosOsArquivos, indiceDoArquivo, extensao):
    retorno = {}
    textoCompleto = scraperwiki.scrape(urlComumAtodosOsArquivos + str(indiceDoArquivo) + extensao)
    retorno['textoCompleto'] = textoCompleto
    #print textoCompleto
    ################################################################
    ...
    retorno['linhasDoTXT'] = linhasDoTXT
    qtdeLinhasDoTXT = len(linhasDoTXT)
    retorno['qtdeLinhasDoTXT'] = qtdeLinhasDoTXT
    ########################################################
...
indiceArquivo = 0
for indiceArquivo in range(1,9):
    print "rodando o arquivo da url: " + url_base + str(indiceArquivo) + ".txt"
    infoArquivo = {}
    infoArquivo = inicializaUmArquivo(url_base, indiceArquivo, '.txt')

    textoCompleto = infoArquivo['textoCompleto']
    linhasDoTXT = infoArquivo['linhasDoTXT']
    qtdeLinhasDoTXT = infoArquivo['qtdeLinhasDoTXT']
                ...
                if not numeroDaEmenda.isdigit(): #Caso tipo o da emenda No 112
                    ...
                    if not numeroDaEmenda.isdigit(): #Caso tipo o da emenda No 104
                        ...
                        if not numeroDaEmenda.isdigit(): #Caso tipo o das emendas No 109,200
                            ...
                            numeroDaEmenda = ''.join(numeroDaEmenda.split()) #Retira espaços vazios da string
                            autorDaEmenda = linhasDoTXT[linhaInicialDeUmaEmenda-1] # O nome do autor da Emenda está na linha anterior da linha Inicial de Emenda
